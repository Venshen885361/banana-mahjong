...
規則書沒有、由本專案追加的特殊役，全部可在 `banana/yaku.py` 找到並自行增刪：

| 役種 | 倍數 | 條件 | 為什麼難 |
|---|---|---|---|
| **BANANA** | 役滿 | 同時擁有 `BBB`、`AAA`、`NNN` 三組刻子（或槓），剩下一組面子 + 雀頭任意 | B 全局只有 3 張，`BBB` 必定用光 |
| **Rush A** | 三倍役滿（副露減一倍） | 把牌山全部 **13 張 A** 收齊 + 任意 1 張 | 13 張 A 湊不出雀頭，所以是獨立和牌型；要吃掉全場所有 A |
| **七連對** | 兩倍役滿 | 七對子，且七組對子的字母**連續**（例：`HH II JJ KK LL MM NN`） | 七對子本身就難，還要連號 |
| **QUIZ** | 兩倍役滿 | 和牌中 `Q`、`U`、`I`、`Z` 四個字母**各至少 2 張** | Q 與 Z 全局各只有 2 張，兩者都必須用光 |

`Rush A` 與 `Rush E` 一樣不走「4 面子 + 1 雀頭」，在 `hand.py::winning_decompositions` 以獨立 `form` 判定，所以聽牌／待張／流局聽牌判定都會自動跟著生效。
