...
遊戲中左下角的 🙂 按鈕會開啟表情選單，送出後會在你座位上方浮出氣泡，所有人都看得到。

### 自訂表情圖片

**把圖檔丟進 `server/emotes/` 就好**，不需要上傳、不需要重開 server，重新整理遊戲頁面就會出現。

| 項目 | 說明 |
|---|---|
| 支援格式 | `.png` `.jpg` `.jpeg` `.gif` `.webp` `.svg` |
| 顯示名稱 | 檔名，開頭數字只用來排序：`01-讚.png` → 顯示「讚」 |
| 建議尺寸 | 128×128 左右，越小載入越快 |
| 數量上限 | 60 個（`emotes.py::MAX_EMOTES`） |

沒有放任何圖檔時，內建的 12 個文字/emoji 表情照樣可用。

### 防刷屏

- 每人 **2 秒冷卻**，server 端強制（`room.py::EMOTE_COOLDOWN`）
- 每位玩家可以點對手名字旁的 🔔 **單獨靜音**某個座位，只影響自己的畫面，設定存在瀏覽器
- 房主可以在遊戲中直接關閉整房的表情

> 安全性：client 只能送出 `/api/emotes` 列出來的 id，server 用白名單比對，塞路徑（`i:../../etc/passwd`）會被擋掉。
...
- **建房時**在大廳直接拉
- **遊戲中**房主點右上角「設定」隨時改，下一個決策點生效，不會打斷現在這一手
- 內建三組 preset：快打 10/5、標準 30/10、悠閒 60/20
- 「不限時」會關掉所有逾時自動打牌，適合跟朋友慢慢玩

逾時的行為：輪到你打牌時逾時 → 自動切掉剛摸的牌；鳴牌階段逾時 → 自動 pass。
